%% SpaRSA_Models Handle
function [ models ] = SpaRSA_Models()
    models.Elastic_Net = @Elastic_Net;
    models.Group_LASSO = @Group_LASSO;
    models.Sparse_Group_LASSO = @Sparse_Group_LASSO;
end
...
psi = @(x,lambda) sparse_group_soft_threshold(x,lambda, alpha, groups);
phi = @(x) sparse_group_l2norm(x, alpha, groups);
% Run SPARSA
...
mse = mean(y_hat ~= y_test); % Calcukate MSE
end
...
% END SpaRSA Models
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Begin Prox Operators
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%% Sparse Group Soft Thresholding Operator
% [(1-a)L1/L2 + aL1] Prox Operator
function [ w ] = sparse_group_soft_threshold( w, lambda, alpha, groups )
% Assign Regulrization
l1 = alpha * lambda;
lg = (1- alpha) * lambda;
% Shrink With Soft & Group Soft Thresholding
if alpha == 0
    w =group_soft_threshold(w, lg, groups);
elseif alpha == 1
...
%% L1/L2 + L1 Norm
function [ y ]= sparse_group_l2norm(w, alpha, groups)
num_groups = size(groups, 2);
y = 0;
groups = (linspace(1, num_groups, num_groups)*groups')';
if alpha ~= 1
    for i=1:num_groups
        y = y + norm(w(groups == i),2);
    end
end
y = ((1-alpha)*y) + (alpha*sum(abs(w(:)))); % Add in L1
end
...
% if psi exist, phi must also exist
if (psi_ok == 1)
